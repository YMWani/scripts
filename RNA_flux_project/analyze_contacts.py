...
def fast_pbc_distance_matrix(chain1_pos, chain2_pos, box_size):
    """
    Efficiently calculate all pairwise distances with periodic boundary conditions.
    ...
    dist_matrix : numpy.ndarray
        Matrix of shape (Nm_chain1, Nm_chain2) containing minimum image distances
    """    
...
def generate_distance_threshold_matrix(chain1_type, chain2_type, sigma_vals):
    """
    Generate a distance threshold matrix based on the atom types and their sigma values.
    ...
    for i in range(chain1_type.shape[0]):
        for j in range(chain2_type.shape[0]):
            ...
            distance_threshold[i, j] = 1.2 * sigma
...
def generate_contact_map_intramolecular(chain_positions, box_size, chain_atom_types, sigma_vals):
    ...
    for idx1, chain1 in enumerate(chain_positions):
...
for i, tstep in enumerate(tqdm(range(num_timesteps), desc="Computing host chain intramolecular contact map"), start=1):
    if i % 100 == 0:
...
np.savetxt("contact_map_host-host.dat", contact_map, fmt="%.4f")
