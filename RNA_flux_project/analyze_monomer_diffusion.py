...
"""
From the trajectory of the guest chains, we want to measure the diffusion of the individual 
monomers of the guest chains when they are inside the condensate.

Algorithm:
1. Load the trajectory of the guest chains
2. For each monomer of the guest chains, determine the timesteps when it is inside the condensate
3. For the timesteps when the monomer is inside the condensate, compute the mean squared displacement (MSD)
4. Average the MSD over all monomers to get the overall MSD of the guest chain monomers inside the condensate
"""
...
if __name__=="__main__":
    ...
    actual_timesteps = []
    delta_t = timesteps[1] - timesteps[0] # number of steps
    for i in range(len(timesteps)):
        actual_timesteps.append(i * delta_t)
    actual_timesteps = np.array(actual_timesteps)

    # Wrap the positions inside the simulation box - needed to determine if a monomer is inside the condensate
    atom_positions_wrapped = wrap_positions_inside_sim_box(atom_positions, box_sizes)

    # Create empty array to store msd values
    msd_values = np.zeros(actual_timesteps.shape[0])
    msd_counts = np.zeros(actual_timesteps.shape[0]) # To count the number of contributions to the MSD at each timestep
    msd_timesteps = actual_timesteps*1e-5 # Convert fs to ns


    print("\nAnalyzing monomer diffusion inside the condensate")
    # Loop over all atoms (monomers)
    for i in tqdm(range(atom_positions.shape[1])):
        atom_traj_unwrapped = atom_positions[:,i,:] # [#frames, 3]
        atom_traj_wrapped = atom_positions_wrapped[:,i,:] # [#frames, 3]

        # Determine the slice indices of the atom trajectory
        delta_z = np.abs((args.condensate_bounds[1] - args.cavity_bounds[1]) / 3)
        slice_indices = determine_slice(args.cavity_bounds, delta_z, atom_traj_wrapped)
        ...
        for start_idx, end_idx in consecutive_ranges:
            ...
            if time_ > 2:
                ...
                atom_sub_traj = atom_traj_unwrapped[start_idx:end_idx].reshape((-1,1,3)) # unwrapped coordinates
    ...
    np.savetxt("monomer_msd.dat", np.column_stack((msd_timesteps, msd_normalized)), header="Time(ns)\tMSD", fmt="%1.5e")